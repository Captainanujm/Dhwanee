...
import { createChallan } from "src/api/billing";
...
export default function NewChallan() {
  ...
  const [addedProducts, setAddedProducts] = useState<
    Array<ProductItemType & { remarks: string }>
  >([]);
  ...
  const handleCreate = () => {
    ...
    if (tokens && customer && addedProducts.length > 0) {
      ...
      createChallan(tokens.access, bill_data)
        .then((data) => {
          ...
          dispatch(
            showSnackbar({
              text: "challan created succesfully",
            })
          );
          setAddedProducts([]);
          setProduct(null);
